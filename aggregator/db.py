...
async def init(
        ...
        connection: str = settings.connection) -> AsyncIOMotorClient:
...
async def insert_logs(
        logs: list | None = None,
        database: str | None = settings.database) -> InsertManyResult | None:
    if logs is None:
        logger.warning(
...
async def get_log(
        log_id: PydanticObjectId | None,
        database: str | None = settings.database) -> JavaLog | None:
    logger.info(
    ...
    try:
        ...
        if result:
            ...
            logger.info(f"When getting {log_id} from db {database} "
                        f"found {result}")
...
async def find_logs(query, sort: str | list[tuple[str, SortDirection]] | None,
                    database: str | None = settings.database) -> list[JavaLog]:
    logger.info(
